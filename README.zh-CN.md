...
未发布候选：**v0.2.1**（`0.2.1+codex.20260714`），正在准备 draft PR。该候选版缩短 3 条 starter prompts，将全部 12 个技能描述改为只描述触发条件，并新增 36 条触发语料和 3 个真实 plugin-eval 基准场景。已发布的 12 个技能 ID 和 Excel 功能范围不变。
...
构建不含开发文档和重复 Agent 镜像的精简 runtime package：
...
对 staged runtime 的静态 plugin-eval 对比实测：`trigger_cost_tokens` 从 1,161 降至 682，减少 41.26%；`invoke_cost_tokens` 从 15,365 降至 14,886（-479）。该结论来自合成/生成的 package analysis，不证明真实任务成功；observed usage（观测用量）是独立证据。可在[任务配方](docs/task-recipes.md)中复现 staging、analysis、comparison 和三场景基准命令。

## 兼容性证据

兼容性必须绑定目标环境。先区分 authoring、automation、consumer 和 recipient 目标，再明确证据属于 **structural evidence（结构证据）**、**runtime capability evidence（运行时能力证据）** 还是 **workbook behavior evidence（工作簿行为证据）**。Windows 桌面版 Excel 可以提供 COM 与 provider 能力探测；macOS、Excel for web、Linux、离线、旧版 Office 和第三方表格软件必须保留各自的宿主证据边界。版本、32/64 位、置信度和目标矩阵见[兼容性边界](docs/compatibility.md)。
