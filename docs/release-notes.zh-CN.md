...
## v0.2.2 - 本地证据工作流与脱敏回归覆盖

当前稳定版：v0.2.2。本次发布提升证据质量，不收集客户数据，也不改变已发布的 12 个技能 ID。

### 变更

- 将可重复的真实/脱敏回归库从 7 个扩展至 12 个案例，覆盖 Power Query 血缘、DAX 度量值改名、转义 MDX 成员、VBA 控件作用域与外部依赖就绪度。
- 新增用户主动启用、仅在本地运行的 JSONL observed-usage 校验器与仅输出聚合结果的汇总器。
- 对 JSON 键名和字段值拒绝路径、凭据与客户制品名；诊断信息会遮蔽敏感键名。
- 新增结构化发布门禁与能力目录覆盖；门禁不要求使用记录或 Excel。
- 插件 manifest 版本升至 `0.2.2+codex.20260714`。

### 边界

系统不会自动收集、上传 observed usage，也不会自动读取工作簿。脱敏回归案例只验证可重复的结构契约；代表性 workbook-behavior 证明仍是独立的本地活动。
